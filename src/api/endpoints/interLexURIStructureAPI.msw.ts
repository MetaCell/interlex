...
export const getPatchEndpointsIlxResponseMock = () => ((() => {
  return {
    status: 200,
    data: {
      "@context": {
...
export const getPatchEndpointsIlxMockHandler = (overrideResponse?: void) => {
  return http.patch('*/:group/:fragPrefId', async () => {
    await delay(1000);
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined ? overrideResponse : getPatchEndpointsIlxResponseMock()),
      {
        status: 200,
        headers: {
          'Content-Type': 'application/json',
